...
const verifyUserCredentials = async ({ email: __email, password: __password }) => {
  ...
  let __query = `SELECT Password FROM xzone.Users WHERE Email='${__email}'`;
  let result;
  ...
  if (compare === true) {
    // Query handle
    __query = `SELECT id FROM xzone.Users WHERE Email='${__email}'`;
    try {
      // Get handle
      [result] = await mysql.connection.query(__query);
      console.log(result)
      return ({ status: true, handle: result[0].id })
    } catch (error) {
...
const verifyAccessToken = (__token) => {
  try {
    console.log(__token)
    const decodedPayload = jwt.verify(__token, "MY_JWT_KEY");
...
const Updatestatus = async (_token)=>{
   let status ="Active";
   let query = `UPDATE xzone.Users Set Status='${status}' where JWT='${_token}'`;
   //console.log(query);
   try{
